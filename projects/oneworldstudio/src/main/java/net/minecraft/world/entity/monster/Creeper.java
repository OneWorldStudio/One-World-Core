...
import com.oneworldstudiomc.paper.event.entity.CreeperIgniteEvent;
...
public class Creeper extends Monster implements PowerableMob {
   ...
   public void readAdditionalSaveData(CompoundTag p_32296_) {
      ...
      if (p_32296_.getBoolean("ignited")) {
         this.entityData.set(DATA_IS_IGNITED, true); // Paper api - set directly to avoid firing event
      }
   ...
   public void explodeCreeper() {
      if (!this.level().isClientSide) {
         ...
         if (!event.isCancelled()) {
            this.dead = true;
            this.level().mohist$pFire(event.getFire());
