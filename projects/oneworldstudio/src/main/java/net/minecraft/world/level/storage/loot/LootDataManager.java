...
public class LootDataManager implements PreparableReloadListener, LootDataResolver {
   ...
   private void apply(Map<LootDataType<?>, Map<ResourceLocation, ?>> p_279426_) {
      ...
      validationcontext.getProblems().forEach((p_279487_, p_279312_) -> {
         if (false) LOGGER.warn("Found loot table element validation problem in {}: {}", p_279487_, p_279312_);
      });
      this.elements = map;
      this.typeKeys = builder1.build();
      // CraftBukkit start - build a reversed registry map
      // Mohist start
      this.elements.forEach((key, lootTable) -> {
         if (object instanceof LootTable table) {
            table.craftLootTable = key.location();
         }
      });
      // Mohist end
      // CraftBukkit end
