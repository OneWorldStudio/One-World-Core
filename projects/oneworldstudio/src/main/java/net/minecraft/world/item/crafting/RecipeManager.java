...
public class RecipeManager extends SimpleJsonResourceReloadListener {
   ...
   protected Map<RecipeType<?>, Object2ObjectLinkedOpenHashMap<ResourceLocation, Recipe<?>>> map1 = Maps.newHashMap();
   ...
   protected void apply(Map<ResourceLocation, JsonElement> p_44037_, ResourceManager p_44038_, ProfilerFiller p_44039_) {
      ...
      for(Map.Entry<ResourceLocation, JsonElement> entry : p_44037_.entrySet()) {
         ...
         try {
            ...
            ((ImmutableMap.Builder)map.computeIfAbsent(recipe.getType(), (p_44075_) -> ImmutableMap.builder())).put(resourcelocation, recipe);
   ...
   public <C extends Container, T extends Recipe<C>> Optional<T> getRecipeFor(RecipeType<T> p_44016_, C p_44017_, Level p_44018_) {
      Optional<T> optional = this.byType(p_44016_).values().stream().filter((p_220266_) -> {
   ...
   public boolean removeRecipe(ResourceLocation mcKey) {
      // Create a temporary mutable map for modification
      Map<RecipeType<?>, Map<ResourceLocation, Recipe<?>>> mutableRecipes = new HashMap<>(this.recipes);
      boolean removed = false;

      for (Map.Entry<RecipeType<?>, Map<ResourceLocation, Recipe<?>>> entry : mutableRecipes.entrySet()) {
         RecipeType<?> type = entry.getKey();
         Map<ResourceLocation, Recipe<?>> recipeMap = new HashMap<>(entry.getValue());

         if (recipeMap.remove(mcKey) != null) {
            removed = true;
            mutableRecipes.put(type, Collections.unmodifiableMap(recipeMap));
         }
      }
