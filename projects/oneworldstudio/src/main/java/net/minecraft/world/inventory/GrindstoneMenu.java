...
public class GrindstoneMenu extends AbstractContainerMenu {
   ...
   private void createResult() {
      ...
      if (!flag) {
         PrepareGrindstoneEvent event = new PrepareGrindstoneEvent(getBukkitView(), CraftItemStack.asCraftMirror(ItemStack.EMPTY).clone());
         event.getView().getPlayer().getServer().getPluginManager().callEvent(event);
         this.resultSlots.setItem(0, CraftItemStack.asNMSCopy(event.getResult()));
      } else {
         ...
         if (j > itemstack2.getMaxStackSize()) {
            ...
            PrepareGrindstoneEvent event = new PrepareGrindstoneEvent(getBukkitView(), CraftItemStack.asCraftMirror(this.removeNonCurses(itemstack2, i, j)).clone());
            event.getView().getPlayer().getServer().getPluginManager().callEvent(event);
            this.resultSlots.setItem(0, CraftItemStack.asNMSCopy(event.getResult()));
         }
      }
