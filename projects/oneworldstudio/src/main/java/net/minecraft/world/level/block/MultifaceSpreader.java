   ...
   public interface SpreadConfig {
      ...
      default boolean placeBlock(LevelAccessor p_221702_, MultifaceSpreader.SpreadPos p_221703_, BlockState p_221704_, boolean p_221705_) {
         ...
         if (blockstate != null) {
            ...
            return org.bukkit.craftbukkit.v1_20_R1.event.CraftEventFactory.handleBlockSpreadEvent(p_221702_, p_221703_.source(), p_221703_.pos(), blockstate, 2); // CraftBukkit
         } else {
            return false;
         }
      }
   ...
   public static enum SpreadType {
      ...
      SAME_PLANE {
         public MultifaceSpreader.SpreadPos getSpreadPos(BlockPos p_221758_, Direction p_221759_, Direction p_221760_) {
            return new MultifaceSpreader.SpreadPos(p_221758_.relative(p_221759_), p_221760_, p_221758_);
         }
      },
      WRAP_AROUND {
         public MultifaceSpreader.SpreadPos getSpreadPos(BlockPos p_221765_, Direction p_221766_, Direction p_221767_) {
            return new MultifaceSpreader.SpreadPos(p_221765_.relative(p_221766_).relative(p_221767_), p_221766_.getOpposite(), p_221765_);
         }
      };
